...
namespace Db4o.Internal.Marshall
{
	...
	{
		private readonly Db4o.Internal.ClassMetadata _classMetadata;
		...
		public ObjectHeader(Db4o.Internal.ClassMetadata classMetadata, IReadWriteBuffer
			 reader) : this(null, classMetadata, reader)
		{
		}

		private ObjectHeader(ObjectContainerBase container, Db4o.Internal.ClassMetadata
		...
		public static Db4o.Internal.Marshall.ObjectHeader Defrag(DefragmentContextImpl
			 context)
		{
			ByteArrayBuffer source = context.SourceBuffer();
			ByteArrayBuffer target = context.TargetBuffer();
			Db4o.Internal.Marshall.ObjectHeader header = new Db4o.Internal.Marshall.ObjectHeader
				(context.Services().SystemTrans().Container(), null, source);
			int newID = context.Mapping().StrictMappedID(header.ClassMetadata().GetID());
			Db4o.Internal.Marshall.SlotFormat slotFormat = header.SlotFormat();
		...
		private Db4o.Internal.Marshall.SlotFormat SlotFormat()
		{
			return Db4o.Internal.Marshall.SlotFormat.ForHandlerVersion(HandlerVersion
				());
		}
		...
		public static Db4o.Internal.Marshall.ObjectHeader ScrollBufferToContent
			(LocalObjectContainer container, ByteArrayBuffer buffer)
		{
			return new Db4o.Internal.Marshall.ObjectHeader(container, buffer);
		}
	}
}
