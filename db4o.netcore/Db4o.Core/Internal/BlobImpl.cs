...
namespace Db4o.Internal
{
	...
	public class BlobImpl : IBlob, System.ICloneable, IDb4oTypeImpl
	{
		...
		public virtual void ReadFrom(Sharpen.IO.File file)
		{
			if (!file.Exists())
			{
				throw new IOException(Db4o.Internal.Messages.Get(41, file.GetAbsolutePath
					()));
			}
		...
		public virtual Sharpen.IO.File ServerFile(string promptName, bool writeToServer)
		{
				...
				if (fileName == null)
				{
					...
					while (new Sharpen.IO.File(path, tryPath).Exists())
					{
						...
						if (i == 99)
						{
							...
							throw new IOException(Db4o.Internal.Messages.Get(40));
						}
					}
					fileName = tryPath;
					lock (i_stream.Lock())
			...
			else
			{
				if (fileName == null)
				{
					throw new IOException(Db4o.Internal.Messages.Get(38));
				}
			}
			...
			if (!writeToServer)
			{
				...
				{
					throw new IOException(Db4o.Internal.Messages.Get(39));
				}
			}
		...
		public virtual void WriteTo(Sharpen.IO.File file)
		{
			...
			{
				throw new IOException(Db4o.Internal.Messages.Get(43));
			}
			if (i_stream.IsClient)
			{
				i_file = file;
