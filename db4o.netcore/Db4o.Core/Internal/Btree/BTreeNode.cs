...
namespace Db4o.Internal.Btree
{
	...
	public sealed class BTreeNode : LocalPersistentBase
	{
		...
		public BTreeNode(Transaction trans, Db4o.Internal.Btree.BTreeNode firstChild
			, Db4o.Internal.Btree.BTreeNode secondChild) : this(firstChild._btree
			, 2, false, 0, 0, 0)
		{
		...
		/// </returns>
		public Db4o.Internal.Btree.BTreeNode Add(Transaction trans, IPreparedComparison
			 preparedComparison, object obj)
		{
			...
			{
				Db4o.Internal.Btree.BTreeNode childNode = Child(reader, s.Cursor());
				Db4o.Internal.Btree.BTreeNode childNodeOrSplit = childNode.Add(trans, 
			 ...
			 preparedComparison, ByteArrayBuffer reader, int index)
		{
			...
			Db4o.Internal.Btree.BTreeNode node = PreviousNode();
			if (node != null)
			{
		...
		internal Db4o.Internal.Btree.BTreeNode Child(int index)
		{
			if (_children[index] is Db4o.Internal.Btree.BTreeNode)
			{
				return (Db4o.Internal.Btree.BTreeNode)_children[index];
			}
			return ProduceChild(index, ((int)_children[index]));
		}

		internal Db4o.Internal.Btree.BTreeNode Child(ByteArrayBuffer reader, int
			 index)
		{
		...
		private Db4o.Internal.Btree.BTreeNode ProduceChild(int index, int childID
			)
		{
			Db4o.Internal.Btree.BTreeNode child = _btree.ProduceNode(childID);
			if (_children != null)
			{
		...
		private int ChildID(int index)
		{
			if (ChildLoaded(index))
			{
				return ((Db4o.Internal.Btree.BTreeNode)_children[index]).GetID();
			}
		...
		private bool ChildLoaded(int index)
		{
			...
			return _children[index] is Db4o.Internal.Btree.BTreeNode;
		}
		...
		private bool ChildCanSupplyFirstKey(int index)
		{
			...
			return ((Db4o.Internal.Btree.BTreeNode)_children[index]).CanWrite();
		}
		...
		public override bool Equals(object obj)
		{
			...
			if (!(obj is Db4o.Internal.Btree.BTreeNode))
			{
				return false;
			}
			Db4o.Internal.Btree.BTreeNode other = (Db4o.Internal.Btree.BTreeNode
				)obj;
		...
		public override void Free(LocalTransaction trans)
		{
			...
			if (!IsRoot())
			{
				Db4o.Internal.Btree.BTreeNode parent = _btree.ProduceNode(_parentID);
		...
		internal void HoldChildrenAsIDs()
		{
			...
			for (int i = 0; i < _count; i++)
			{
				if (_children[i] is Db4o.Internal.Btree.BTreeNode)
				{
					_children[i] = ((Db4o.Internal.Btree.BTreeNode)_children[i]).GetID();
				}
			}
		}

		private void RemoveChild(Transaction trans, Db4o.Internal.Btree.BTreeNode
			 child)
		{
			PrepareWrite(trans);
			SetStateDirty();
		...
		private void KeyChanged(Transaction trans, Db4o.Internal.Btree.BTreeNode
			 child)
		{
			PrepareWrite(trans);
			SetStateDirty();
			 ...
			 obj, int index)
		{
			...
			Db4o.Internal.Btree.BTreeNode node = NextNode();
			if (node == null)
			{
				return;
			}
		...
		private Db4o.Internal.Btree.BTreeNode Split(Transaction trans)
		{
			Db4o.Internal.Btree.BTreeNode res = new Db4o.Internal.Btree.BTreeNode
		...
		public Db4o.Internal.Btree.BTreeNode PreviousNode()
		{
			if (_previousID == 0)
			{
				return null;
		...
		public Db4o.Internal.Btree.BTreeNode NextNode()
		{
			if (_nextID == 0)
			{
				return null;
		...
		public override void WriteThis(Transaction trans, ByteArrayBuffer buffer)
		{
			...
			else
			{
				for (int i = 0; i < _count; i++)
				{
					...
					{
						Db4o.Internal.Btree.BTreeNode child = (Db4o.Internal.Btree.BTreeNode
							)_children[i];
		...
		public void DebugLoadFully(Transaction trans)
		{
			...
			for (int i = 0; i < _count; ++i)
			{
				...
				((Db4o.Internal.Btree.BTreeNode)_children[i]).DebugLoadFully(trans);
			}
		}
		...
		public override Db4o.Internal.Slots.SlotChangeFactory SlotChangeFactory
			()
		{
